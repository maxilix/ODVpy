...
class Sublayer(ReadableFromStream):
    ...
    def __init__(self, allowed_area, disallowed_area_list, segment_list):
        self.allowed_area = allowed_area
        self.segment_list = segment_list
        self.disallowed_area_list = disallowed_area_list
        self.objectA_list_list = []

    @classmethod
    def from_stream(cls, stream):
        allowed_area = stream.read(Area)
        nb_segment = stream.read(UShort)
        segment_list = [stream.read(Segment) for _ in range(nb_segment)]
        nb_area = stream.read(UShort)
        disallowed_area_list = [stream.read(Area) for _ in range(nb_area)]
        return cls(allowed_area, disallowed_area_list, segment_list)
...
class ObjectA(ReadableFromStream):

    def __init__(self, b0, coor, b1, objectB_index_list):
        self.b0 = b0
        self.coor = coor
        self.b1 = b1
        self.objectB_index_list = objectB_index_list
    ...
    def from_stream(cls, stream):
        ...
        objectB_index_list_length = stream.read(UShort)
        objectB_index_list = [stream.read(UShort) for _ in range(objectB_index_list_length)]
        return cls(b0, coor, b1, objectB_index_list)


class ObjectB(ReadableFromStream):

    def __init__(self, indexes_1, indexes_2, a, b, objectC_index_list):
    ...
    def from_stream(cls, stream):
        ...
        a = stream.read(UShort)
        b = stream.read(UShort)
        ...
        return cls(indexes_1, indexes_2, a, b, objectC_index_list)
...
class Motion(Section):
        ...
        while True:
            ...
            if nb_layer == 0:
                ...
                for layer_index in range(nb_layer):
                    ...
                    nb_sublayer = stream.read(UShort)
                    # print("sublayer ", end='')
                    assert nb_sublayer == len(self.layer_list[layer_index].sublayer_list)
                    for sublayer_index in range(nb_sublayer):
                        # print(f"\n          ----- {sublayer_index=} -----\n               ", end='')
                        nb_area = stream.read(UShort)
                        # print("area ", end='')
                        assert nb_area == (1 + len(
                            self.layer_list[layer_index].sublayer_list[sublayer_index].disallowed_area_list))
                        for area_index in range(nb_area):
                            # print(f"\n               ----- {area_index=} -----\n                    ", end='')
                            nb_A = stream.read(UShort)
                            self.layer_list[layer_index].sublayer_list[sublayer_index].objectA_list_list.append(
                                [stream.read(ObjectA) for _ in range(nb_A)])
                            # last_index_read = self.layer_list[layer_index].sublayer_list[sublayer_index].a_list[-1].objectB_index_list[-1]

                # print(f"\n     ", end='')
                nb_B = stream.read(UShort)
                # assert (last_index_read+1)==nb_B
                self.objectB_list = [stream.read(ObjectB) for _ in range(nb_B)]
                stream.read_raw()
                break
                # print(f"\n     ", end='')
                # nb_C = stream.read(UShort)
                # self.objectC_list = stream.read_raw()  # [stream.read(ObjectC) for _ in range(nb_C)]
    ...
    def disallow_QPolygonF(self, layer_index, sublayer_index):
        return [QPolygonF(
            ...
            for area in self.layer_list[layer_index].sublayer_list[sublayer_index].disallowed_area_list]
    ...
    def print(self):
        for i, layer in enumerate(self.layer_list):
            ...
            for j, sublayer in enumerate(layer.sublayer_list):
                ...
                print(f"    nb_point_in_allow_area = {i_to_hsi(len(sublayer.allowed_area.coor_list))}")
                print(f"    nb_segment={i_to_hsi(len(sublayer.segment_list))}")
                print(f"    nb_disallowed_area={i_to_hsi(len(sublayer.disallowed_area_list))}")
                for disallowed_area in sublayer.disallowed_area_list:
                    print(f"      nb_point_in_disallow_area = {i_to_hsi(len(disallowed_area.coor_list))}")

    def get_ObjectA(self, indexes):
        return self.layer_list[indexes[0]].sublayer_list[indexes[1]].objectA_list_list[indexes[2]][indexes[3]]
