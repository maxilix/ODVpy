...
class ScbQuad(RWStreamable):
    @classmethod
    def from_stream(cls, stream: ReadStream, *, i) -> Self:
        rop = cls()
        print(f"    --{i=}--")
...
class ScbClass(OdvObject):
    ...
    def from_stream(cls, stream: ReadStream, *, parent) -> Self:
        ...
        rop.nb_function = int(stream.read(String, end="\n"))
        print(f"  {rop.nb_function=}")

        for _ in range(rop.nb_function):
        ...
        rop.quad = [stream.read(ScbQuad, i=i) for i in range(nb_quad)]
